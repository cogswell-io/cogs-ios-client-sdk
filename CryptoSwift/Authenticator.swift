...
public enum Authenticator {
    ...
    */
    ...
    case HMAC(key: [UInt8], variant:GambitSDK.HMAC.Variant)
    ...
    */
    public func authenticate(message: [UInt8]) throws -> [UInt8] {
        ...
        case .Poly1305(let key):
            guard let auth = GambitSDK.Poly1305.authenticate(key: key, message: message) else {
                throw Error.AuthenticateError
            }
            return auth
        case .HMAC(let key, let variant):
            guard let auth = GambitSDK.HMAC.authenticate(key: key, message: message, variant: variant) else {
                throw Error.AuthenticateError
            }
            return auth
        }
